...
{
	// use helper method
	BuildBuffer(vertices, numVertices, indices, numIndices, device);
}
...
Mesh::Mesh(const char * objFile, ID3D11Device * device)
{
	...
	BuildBuffer(&verts[0], vertCounter, &indices[0], vertCounter, device);
}
...
void Mesh::BuildBuffer(Vertex * vertices, int numVertices, unsigned int * indices, int numIndices, ID3D11Device * device)
{
	...
	ibd.ByteWidth = sizeof(int) * numIndices;         // number of indices in the buffer
